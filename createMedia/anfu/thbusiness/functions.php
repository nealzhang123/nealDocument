...
var load_image = '<img src="<?php bloginfo('template_url');?>/images/timg.gif" class="load_image" />';
</script>
<?php
}
...
function about_form_save() {
    ...
    $subject = '安富裝飾網站聯絡我們表單';
    $message = 'Name : ' . esc_attr( $data['name'] ) . '<br />';
    $message .= 'Email : ' . esc_attr( $data['email'] ) . '<br />';
    $message .= 'Subject : ' . esc_attr( $data['subject'] ) . '<br />';
    $message .= 'Message: ' . esc_attr( $data['message'] ) . '<br />';
...
add_action('wp_ajax_search-select-project-ajax', 'search_select_project_ajax');
add_action('wp_ajax_nopriv_search-select-project-ajax', 'search_select_project_ajax');

function search_select_project_ajax(){
	include_once( 'project_ajax_content.php' );
