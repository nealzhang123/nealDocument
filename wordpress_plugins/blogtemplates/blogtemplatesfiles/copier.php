...
class NBT_Template_copier {
    ...
    public function stacktech_copy_dir($from, $to, $skip_list = array() ) {
        ...
        foreach ( (array) $dirlist as $filename => $fileinfo ) {
            ...
            if ( 'f' == $fileinfo['type'] ) {
                ...
                //insert the file into oss_image_upload_url table start
                $todir       = $to. $filename;
                ...
                //insert the file into oss_image_upload_url table end
                ...
                $result = $this->stacktech_copy_dir($from . $filename, $to . $filename, $sub_skip_list);
    ...
    public function copy_files() {
        ...
        if ( is_dir( $dir_to_copy ) ) {
            ...
            if ($result) {
                ...
                $result = $this->stacktech_copy_dir( $dir_to_copy, $dir_to_copy_into, $skip_list );

                // copy file to oss start
                $url ='curl --max-time 120 '. network_site_url() .'wp-content/plugins/00-stacktech-upload-file-to-oss/upload-delete.php?action=add > /dev/null & ';
                exec($url);
                // $file_upload = new file_Upload_To_Oss();
                // $rseult = $file_upload->uploadfiletooss($fromdir,$todir);
                 // copy file to oss end 
