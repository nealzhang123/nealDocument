					            ...
					            <?php if ( apply_filters( 'nbt_activate_categories_feature', true ) ): ?>
						            ...
						            <div id="postbox-container-1" class="postbox-container">
										<div id="side-sortables" class="meta-box-sortables ui-sortable">
											...
											<div id="categorydiv" class="postbox ">
												<div class="handlediv" title=""><br></div><h3 class="hndle"><span><?php _e( '认证选项' ); ?></span></h3>
												<div class="inside">
													<div id="taxonomy-category" class="categorydiv">
														<div id="category-all" class="tabs-panel">
															<ul id="templateauthenticatelist" class="categorychecklist form-no-clear">
															<?php  
															$template = $model->get_template( $t );
															$value = maybe_unserialize($template['verify_type']);
															$val = $value['template_authenticate'];
														
															 ?>
																<li id="template-1">
																<label class="selectit">
																<input  type="checkbox" <?php checked( in_array( 'public', $val ) ); ?>  value="public" class="selectit_1" id="public" >公众
																</label>
																</li>
																<li id="template-2">
																<label class="selectit">
																<input  type="checkbox" <?php checked( in_array( 'phone', $val ) ); ?> value="phone" class="selectit_1" id="phone">手机认证
																</label>
																</li>
																<li id="template-3">
																<label class="selectit">
																<input  type="checkbox" <?php checked( in_array( 'personal', $val ) ); ?> class="selectit_1" value="personal">个人认证
																</label>
																</li>
																<li id="template-4">
																<label class="selectit">
																<input  type="checkbox" <?php checked( in_array( 'company', $val ) ); ?> class="selectit_1" value="company">公司认证
																</label>
																</li>
															</ul>
															<input type="hidden" value="" id="hidden_1" name="template_authenticate" >
														</div>
													</div>
												</div>
											</div>
        ...
        */
        ...
        function admin_options_page_posted() {
            ...
            if( $save_template ) {
                ...
                //change it to post
                $template_authenticate = array();
                if( isset($_POST['template_authenticate']) && !empty($_POST['template_authenticate']) ){
                	$template_authenticate = explode(',', $_POST['template_authenticate']);
                	array_pop($template_authenticate);
                }else{
                	$template = $model->get_template( $t );
					$value = maybe_unserialize($template['verify_type']);
					$val = $value['template_authenticate'];
					$template_authenticate = $val;
                }
                
                $verify_type = array(
                	'template_authenticate' => $template_authenticate,
                	);
                ...
                $template_id = $model->add_template( $blog_id, $name, $description, $settings, $verify_type );
